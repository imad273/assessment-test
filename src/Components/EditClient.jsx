...
function EditClient({ displayError, displaySucc, getAllUsersData, closePage, person_id }) {
  ...
  const getProfileData = async () => {
    ...
    setPersonID(response.data.client.person.id);
    setEmail(response.data.client.person.email);
    setFirstName(response.data.client.person.first_name);
    setLastName(response.data.client.person.last_name);
    setCity(response.data.client.person.city);
    setAddress(response.data.client.person.address);
    setPhone(response.data.client.person.phone01);
  }
  ...
  const updateProfile = async () => {
    ...
    if (Email === "" || FirstName === "" || LastName === "" || Phone === "" || City === "" || Address === "") {
      ...
      if (response.data.updateClient.status === false) {
