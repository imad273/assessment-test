...
function EditProfile({ displayError, displaySucc, closePage }) {
    ...
    <div className='fixed top-0 left-0 min-h-screen w-full flex justify-center items-center bg-white/50 backdrop-blur-sm rounded z-20'>
      <div className='bg-white shadow p-2'>
        <div className='flex justify-between items-center'>
          <h3 className='text-2xl md:text-3xl font-medium'>Edit Profile</h3>
        ...
        <form className='flex flex-col py-2' onSubmit={(e) => e.preventDefault()}>
          ...
          <div className='my-0.5 flex flex-col'>
            <label htmlFor="password" className='pb-0.5 md:text-lg font-medium'>Password</label>
            <input type="password" value={Password} onChange={(e) => setPassword(e.target.value)} name='password' placeholder='New password' className='outline-none p-1 border border-[#F3F3F3] rounded' />
          </div>
          ...
          <div className='my-0.5 flex flex-col'>
            ...
            <select value={City} name="city" className="outline-none p-1 border border-[#F3F3F3] rounded" onChange={(e) => { setCity(e.target.value); }}>
              {cities.map((city, key) => {
                return (
