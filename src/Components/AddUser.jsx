...
function AddUser({ displayError, displaySucc, closePage, getAllUsersData }) {
  ...
  const CreateUser = async () => {
    ...
    if (UserName === "" || Email === "" || Password === "" || FirstName === "" || LastName === "" || Phone === "" || City === "" || Address === "") {
      ...
      if (response.errors) {
        ...
        displaySucc("User created successfully")
        closePage(false);
        getAllUsersData();
      }
  ...
  return (
    <div className='fixed top-0 left-0 min-h-screen w-full  flex justify-center items-center bg-white/50 backdrop-blur-sm rounded z-20'>
      <div className='bg-white shadow p-2 w-full h-full md:w-3/6 md:h-auto'>
        <div className='flex justify-between items-center'>

          <h3 className='text-2xl md:text-3xl font-medium text-center'>Add new user</h3>
